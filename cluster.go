...
func (c *Cluster) AddNode(node *Node) error {
	...
	if node.ID != c.Node.ID {
		node.api = newRemoteAPI(node.URI, c.internalClient)
	}
	...
	go func() {
		...
		if err := func() error {
			...
			for _, src := range instr.Sources {
				...
				client, err := NewExternalHTTPClient(srcURI.String(), c.internalClient)
				if err != nil {
					return err
				}

				// TODO: convert src.Node to a *Node which has a valid Node.api
				// so that below we can just call node.api.RetrieveSlice()
				// that also allows us to get rid of the client above
				...
				// TODO: this needs to create a Node with client based on URI and call node.backupSliceNode(ctx, index, frame, view, slice)
				rd, err := client.RetrieveSlice(context.Background(), src.Index, src.Frame, src.View, src.Slice)
				if err != nil {
